...
import { useAccount, useChainId, useConfig, useWriteContract, useReadContracts, useWaitForTransactionReceipt } from "wagmi"
...
export default function AirdropForm({ isUnsafeMode, onModeChange }: AirdropFormProps) {
    ...
    const total: number = useMemo(() => calculateTotal(amounts), [amounts]) // 在amounts变化的时候，才会调用calculateTotal(amounts)方法来计算新的total值，否则就会使用上次缓存好的值
    ...
    // 查看当前用户钱包地址account.address（授权人）授权给合约地址tSenderAddress的ERC20代币额度
    async function getApprovedAmount(tSenderAddress: string | null): Promise<number> {
        ...
        // tokenAddress.allowance(account, tSenderAddress)
        // ERC20(tokenAddress).allowance(account.address, tSenderAddress)
